...
class AzureOpenAIIntelligence:
    """
    v18: Integrates Microsoft Azure OpenAI (gpt-oss-120b) for advanced trading logic.
    Provides human-like sentiment analysis on news and complex macro regimes.
    Requires AZURE_OPENAI_ENDPOINT and AZURE_OPENAI_KEY in .env.
    """
    def __init__(self):
        self._endpoint = os.environ.get("AZURE_OPENAI_ENDPOINT", "https://binbot-ai-resource.services.ai.azure.com/openai/v1")
        self._key = os.environ.get("AZURE_OPENAI_KEY", "")
        self._model = os.environ.get("AZURE_OPENAI_MODEL", "gpt-oss-120b")
        self._client = None
        self._cache = {}
        self._cache_s = 3600
        ...
        if self._endpoint and self._key:
            try:
                ...
                log.info("🤖 Azure OpenAI Intelligence initialized successfully.")
            except ImportError:
                log.warning("⚠️ 'openai' SDK missing. Run 'pip install openai'. Azure OpenAI disabled.")
            except Exception as e:
                log.warning(f"⚠️ Failed to initialize Azure OpenAI: {e}")

    def analyze_news(self, headlines: list, symbol: str) -> float:
        """
        Sends headlines to OpenAI and requests a float between -1.0 (bearish) and 1.0 (bullish).
        ...
        headline_text = "\n".join(headlines[:5])
        cache_key = f"{symbol}:{hash(headline_text)}"
        
        cached = self._cache.get(cache_key)
        if cached and time.time() - cached.get("ts", 0) < self._cache_s:
            return cached["score"]
            
        prompt = (
            f"You are an expert quantitative crypto analyst. Review the following recent news headlines related to the crypto market and specifically {symbol}.\n\n"
            f"{headline_text}\n\n"
            "Analyze the sentiment. Return ONLY a single float value between -1.0 (extremely bearish) and 1.0 (extremely bullish). Do not provide any explanation, text, or formatting. Just the float number."
        )
        ...
        try:
            ...
            if match:
                ...
                log.debug(f"Could not extract float from Azure OpenAI response: {result_text}")
                score = 0.0
                
            # Bound the score just in case
            score = max(-1.0, min(1.0, score))
            
            self._cache[cache_key] = {"score": score, "ts": time.time()}
            return score
        except Exception as e:
            log.debug(f"Azure OpenAI API Error: {e}")
            return 0.0
