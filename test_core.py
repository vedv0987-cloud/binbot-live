...
class TestConfigHardening(unittest.TestCase):
    ...
    def test_safe_defaults(self):
        ...
        c = Config()
        # v18.9.16: SL ceiling stays within a SANE range (wider stops are OK — risk-normalize
        # caps the dollar loss regardless of stop width, so the size shrinks for a wide stop).
        self.assertLessEqual(c.MAX_SL_PCT, 0.10, "SL ceiling should stay within a sane range")
        self.assertGreaterEqual(c.MAX_SL_PCT, c.STOP_LOSS_PCT, "ceiling must be >= floor")
